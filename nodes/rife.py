...
from .Rife.vfi_utilities import preprocess_frames, postprocess_frames, generate_frames_rife, logger
from .Rife.trt_utilities import Engine_rife
import folder_paths
import time
...
class RifeTensorrt:
    ...
    def vfi(
        ...
        if (not hasattr(self, 'engine') or self.engine_label != engine):
            self.engine = Engine_rife(engine_path)
