...
def run(image_path, output_path, output_alpha_path, engine_path, process_size=1024, mode="s"):
    runtime = trt.Runtime(logger)
    ...
    if mode == "s":
        infer_single(image_path, output_path, output_alpha_path, engine, context, transformers,
                     numpy_to_pil)
    else:
        ...
        for image in images:
            ...
            output_image_path = os.path.join(output_path, image).replace(".jpg", ".png")
            output_alpha_path = os.path.join(output_alpha_path, image).replace(".jpg", ".png")
            infer_single(image_path, output_image_path, output_alpha_path, engine, context, transformers,
                         numpy_to_pil)
