...
def test2(onnx_path): #测试运行时间-onnx
    input_tensor = torch.randn(1, 3, 1024, 1024)
    evaluate_onnx_models(onnx_path, input_tensor)

def test3(trt_path): #测试运行时间-trt
    input_tensor = torch.randn(1, 3, 1024, 1024)
    evaluate_tensorrt_models(trt_path, input_tensor)
